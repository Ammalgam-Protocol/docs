# Saturation
[Git Source](https://github.com/Ammalgam-Protocol/core-v1/blob/15372364f447bc8a3cfdf61e53c38abb19412d55/contracts/libraries/Saturation.sol)
...
where $$L^2 < X \cdot Y$$ transitions to $$L^2 > X \cdot Y$$. The current value is
50 * BIPS which allows for the premium to grow quickly as the straddle's window of
insolvency grows rapidly once they reach this transition point.
...
function getPenaltySharesPerSatFromLeaf(
    Tree storage tree,
    uint256 leaf
) internal view returns (uint256 penaltyInBorrowLSharesPerSatInQ72);
```
**Parameters**
...
function calcNewAccountPenalty(
    ...
    address account,
    uint256 trancheIndex
) internal view returns (uint256 penaltyInBorrowLShares, uint256 accountTreePenaltyInBorrowLSharesPerSatInQ72);
```
**Parameters**
...
function calcNewPenaltiesGivenTree(
    ...
    uint256 allAssetsBorrowL,
    uint256 allSharesBorrowL
) internal view returns (uint256 penaltyInBorrowLShares, uint256 penaltyInBorrowLSharesPerSatInQ72);
```
**Parameters**
...
function findHighestSetLeafUpwards(
    ...
    uint256 level,
    uint256 nodeIndex
) internal view returns (uint256 highestSetLeaf);
```
**Parameters**
...
function calcStraddlePremiumRatioBips(
    uint256[6] memory userAssets
) internal pure returns (uint256 ratioBips);
```
**Parameters**
...
function liqPriceDividedBySpan(
    ...
    uint256 sqrtPriceSpanQ72
) internal pure returns (uint256 priceQ128);
```
